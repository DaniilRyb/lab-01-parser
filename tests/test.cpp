...
TEST(parser, TheFirstCheckIsValidStudent) {
  student_t st1(json::parse(R"(
 {
  ...
  ASSERT_DOUBLE_EQ(std::any_cast<json>(st1.GetDebt()).get<double>(), 4.25);
}
TEST(parser, TheSecondCheckIsValidStudent) {
  student_t st1(json::parse(R"(
 {
  ...
  ASSERT_EQ(st1.GetName(), "Sidorov Ivan");
  ASSERT_EQ(std::any_cast<json>(st1.GetGroup()).get<std::string>(), "31");
  ASSERT_DOUBLE_EQ(std::any_cast<json>(st1.GetDebt()).get<double>(), 4.25);
  ASSERT_EQ(std::any_cast<json>(st1.GetDebt()).get<std::string>(), "C++");
}
  ...
  student_t st3(json::parse(R"(
{
  ...
  ASSERT_DOUBLE_EQ(std::any_cast<json>(st3.GetDebt()).get<double>(), 3.33);
